...
const MOCK_RECIPE: Recipe = {
  ...
  steps: [
    ...
    { id: 's2', type: 'pasteurization', instruction: 'Pasteurize mix at 85°C for 5 minutes.', targetTemperature: 85 },
    { id: 's3', type: 'aging', instruction: 'Cool rapidly and age for 12 hours at 4°C.', targetTemperature: 4, durationMinutes: 720 },
    { id: 's4', type: 'churning', instruction: 'Churn in batch freezer until desired consistency.' },
  ],
};

const INGREDIENT_GROUP_ORDER: IngredientGroup[] = [
  'liquids', 'sugars', 'stabilizers', 'flavorings', 'solids',
];
...
export default function RecipeOverviewScreen() {
  ...
  const batchSizeNum = useMemo(() => parseFloat(batchSize) || recipe.baseWeightGrams, [batchSize]);
  ...
  return (
    <SafeAreaView style={[styles.safeArea, { backgroundColor: colors.bgPrimary }]}>
      ...
      >
        ...
        {/* ── Header ── */}
        <View style={[styles.recipeHeader, { borderBottomColor: colors.borderSubtle, paddingBottom: spacing.lg, marginBottom: spacing.lg }]}>
          <Text style={[textStyles.recipeNameLarge, { color: colors.textPrimary }]}>
            {recipe.name}
          </Text>
          ...
          {/* Batch size control */}
          <View style={[styles.yieldControl, { backgroundColor: colors.bgSecondary, borderColor: colors.borderDefault, borderRadius: radii.md, marginTop: spacing.md }]}>
            <Text style={[textStyles.label, { color: colors.textSecondary, fontSize: 10 }]}>
              Batch Weight (g)
            </Text>
            <TextInput
              style={[textStyles.body, styles.yieldInput, { color: colors.textAccent }]}
              value={batchSize}
        ...
        <View style={[styles.section, { backgroundColor: colors.bgSecondary, borderRadius: radii.lg, borderColor: colors.borderSubtle, ...(mode === 'dark' ? shadows.card : shadows.cardLight) }]}>
                ...
                <View
                  ...
                  style={[
                    styles.ingredientRow,
                    {
                      ...
                      borderBottomColor: colors.borderSubtle,
                      borderBottomWidth: iIdx < groupData.items.length - 1 ? 1 : 0,
                    }
                  ]}
                >
                  ...
                  <Text style={[textStyles.body, { color: colors.textPrimary, fontWeight: '600', marginHorizontal: spacing.md, minWidth: 70, textAlign: 'right' }]}>
                    {getAdjustedWeight(ing.percentage)}
                    <Text style={{ color: colors.textSecondary }}> g</Text>
                  </Text>
                  <Text style={[textStyles.bodySmall, { color: colors.textSecondary, minWidth: 40, textAlign: 'right' }]}>
                    {ing.percentage}%
                  </Text>
                </View>
              ))}
        ...
        {/* ── Process Flow ── */}
        <Text style={[textStyles.h3, { color: colors.textPrimary, marginTop: spacing.xl, marginBottom: spacing.md }]}>
          Process Flow
        </Text>
        ...
        <View style={{ gap: spacing.sm }}>
          {recipe.steps.map((step, idx) => (
            ...
            >
              {/* Step number circle */}
              <View style={[styles.stepNumberCircle, { backgroundColor: colors.bgTertiary }]}>
                <Text style={[textStyles.stepNumber, { color: colors.textAccent, fontSize: 14 }]}>
                  {idx + 1}
                </Text>
              </View>
              ...
              <View style={styles.stepInfo}>
                ...
                <View style={styles.stepBadges}>
                  {step.targetTemperature !== undefined && (
                    <View style={[styles.metaBadge, { backgroundColor: colors.bgTertiary }]}>
                      <Text style={[textStyles.label, { color: colors.textPrimary, fontSize: 10 }]}>
        ...
        <View style={[
          ...
          <View style={{ flex: 1, marginLeft: spacing.md }}>
            <Text style={[textStyles.body, { color: colors.textAccent, fontWeight: '700' }]}>
              Required Aging: {recipe.agingTimeHours} Hours at 4°C
            </Text>
            <Text style={[textStyles.bodySmall, { color: colors.textSecondary, marginTop: spacing.xxs }]}>
...
const styles = StyleSheet.create({
  ...
  yieldControl: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', paddingHorizontal: 16, paddingVertical: 10, borderWidth: 1, alignSelf: 'flex-start', minWidth: 200 },
  ...
  stepCard: { flexDirection: 'row', alignItems: 'flex-start', borderLeftWidth: 3 },
  stepNumberCircle: { width: 30, height: 30, borderRadius: 15, alignItems: 'center', justifyContent: 'center', marginRight: 14, flexShrink: 0 },
