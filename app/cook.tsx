...
export default function CookScreen() {
  ...
  const targetWeight = recipe.baseWeightGrams * (currentIngredient.percentage / 100);
  ...
  const status: StatusType = isComplete ? 'perfect' : diff < 0 ? 'add' : 'remove';
  ...
  const handleAbort = () => {
    Alert.alert(
      ...
      [
        ...
        { text: 'Abort', style: 'destructive', onPress: () => router.push(`/recipe/${id}`) },
      ]
    );
  };
    ...
    <SafeAreaView style={[styles.safeArea, { backgroundColor: colors.bgPrimary }]}>
      ...
      <View style={[styles.header, { paddingHorizontal: spacing.md, paddingTop: spacing.md }]}>
        <Pressable onPress={handleAbort} style={styles.abortBtn}>
          {({ pressed }) => (
            <Text style={[textStyles.bodySmall, { color: pressed ? colors.error : colors.textSecondary }]}>
              Abort Batch
            </Text>
          )}
        </Pressable>
        ...
        <View style={styles.headerCenter}>
          <Text style={[textStyles.bodySmall, { color: colors.textSecondary, textAlign: 'right' }]} numberOfLines={1}>
            {recipe.name}
          </Text>
          <Text style={[textStyles.label, { color: colors.textAccent, textAlign: 'right', fontSize: 10 }]}>
      ...
      <View style={[styles.main, { paddingHorizontal: spacing.md }]}>
        <View style={[
          ...
          {/* Ingredient label */}
          <Text style={[textStyles.label, { color: colors.textSecondary, fontSize: 10, textAlign: 'center', marginBottom: spacing.xs }]}>
            {currentIngredient.group}
          </Text>
          <Text style={[textStyles.h2, { color: colors.textPrimary, textAlign: 'center', marginBottom: spacing.lg }]}>
          ...
          <View style={styles.weightRow}>
            ...
            <Text style={[textStyles.xl, { color: colors.textSecondary, alignSelf: 'flex-end', marginBottom: 12, marginLeft: 6 }]}>
              g
            </Text>
          </View>

          {/* Target info */}
          <Text style={[textStyles.body, { color: colors.textSecondary, textAlign: 'center', marginBottom: spacing.sm }]}>
            Target:{' '}
          ...
          {/* Progress bar */}
          <View style={[styles.progressTrack, { backgroundColor: colors.bgTertiary, borderRadius: radii.full }]}>
            <Animated.View
              style={[
                styles.progressFill,
                {
        ...
        {/* ── Controls ── */}
        <View style={[styles.controls, { marginTop: spacing.md, gap: spacing.sm }]}>
          <Pressable
            onPress={handleTare}
            style={({ pressed }) => [
              styles.controlBtn,
        ...
        {scaleWeight > targetWeight + 5 && !isAdjusting && (
          <Animated.View style={[
            ...
            <Text style={[textStyles.body, { color: colors.error, fontWeight: '700', marginBottom: spacing.xxs }]}>
              ⚠️ Overweight
            </Text>
            <Text style={[textStyles.bodySmall, { color: colors.textSecondary, marginBottom: spacing.sm }]}>
              Exceeds tolerance by {(scaleWeight - targetWeight).toFixed(1)}g
            </Text>
            <Pressable onPress={() => setIsAdjusting(true)}>
              {({ pressed }) => (
                <Text style={[textStyles.bodySmall, { color: pressed ? colors.error + 'aa' : colors.error, textDecorationLine: 'underline' }]}>
      ...
      <View style={[
        ...
        <Pressable
          ...
          style={({ pressed }) => [
            styles.navBtn,
            { opacity: currentStepIndex === recipe.ingredients.length - 1 ? 0.3 : pressed ? 0.6 : 1 }
          ]}
        >
          <Text style={[textStyles.body, { color: colors.textSecondary }]}>
            Skip →
...
const styles = StyleSheet.create({
  ...
  weightRow:        { flexDirection: 'row', alignItems: 'flex-end', justifyContent: 'center', marginBottom: 12 },
  ...
  bottomNav:        { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', borderTopWidth: 1 },
